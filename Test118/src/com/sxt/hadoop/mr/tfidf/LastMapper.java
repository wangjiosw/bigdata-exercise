...
public class LastMapper extends Mapper<LongWritable, Text, Text, Text> {
	...
	// 存放每条微博的字数
	public static Map<String, Integer> wc = null;
	// 在map方法执行之前
	protected void setup(Context context) throws IOException,
			InterruptedException {
		System.out.println("******************");
		if (cmap == null || cmap.size() == 0 || df == null || df.size() == 0 || wc == null || wc.size() == 0) {
...
//					System.out.println(uri + "-------------");
					if (uri.getPath().endsWith("part-r-00003")) {// 微博总数
						...
						br.close();
					} else if (uri.getPath().contains("weibo2")) {// 词条的DF
						...
						br.close();
					} else if (uri.getPath().contains("weibo4")) {
						...
						while ((line = br.readLine()) != null) {
							String[] ls = line.split("\t");
							wc.put(ls[0], Integer.parseInt(ls[1].trim()));
